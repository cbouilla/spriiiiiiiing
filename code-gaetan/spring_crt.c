...
#if USE_NAIVE_R2
R2poly CX[K/8][256] ALIGN;
#else
v8 BB[K/8][256][N/32] ALIGN;
#endif
R2poly C[K+1][2] ALIGN;			// C[k][1] is inv
int16_t D[K+1][2][N] ALIGN; // D[k][1] is inv

#if USE_CLMUL == 0
// Table based polynomial multiplication
//uint64_t CC[K+1][2][256] ALIGN;			// CC[k][1] is inv
R2poly CC[K+1][2][256] ALIGN;			// CC[k][1] is inv
#endif
...
void initializeB_C ()
{
	...
	for (i = 0; i < K+1; ++i)
	{
		...
		assert (isInvertible(poly));
		C[i][0] = poly;
		C[i][1] = inverseInRing2(poly);
#if USE_CLMUL == 0
		for (int x=0; x<256; x++) {
			union u64 xx = {.u = {x, 0}};
			CC[i][0][x] = multiplyR2PolynomialsInRing(C[i][0], xx.v);
			CC[i][1][x] = multiplyR2PolynomialsInRing(C[i][1], xx.v);
		}
#endif
	}
...
void initializeXX ()
{
	...
	for (int kk=0; kk<K/8; kk++) {
		CX[kk][0] = one.v;
		for (int j=0; j<8; j++) {
			CX[kk][1<<j] = C[8*kk+j][0];
		}
		// Compute by linearity
		for (int k = 1; k<256; k++) {
			int msb = 1<<(__builtin_ffs(k)-1);
			assert(k&msb);
			CX[kk][k] = multiplyR2PolynomialsInRing(CX[kk][k-msb], CX[kk][msb]);
		}
	}

	for (int k=0; k<256; k++) {
		CX[0][k] = multiplyR2PolynomialsInRing(CX[0][k], C[K][0]);
	}
	...
	for (int kk=0; kk<K/8; kk++) {
		for (int i=0; i<N/32; i++) {
			BB[kk][0][i] ^= BB[kk][0][i];
			for (int j=0; j<8; j++) {
				BB[kk][1<<j][i] = vB[8*kk+j][i];
			}
			// Compute by linearity
			for (int k = 1; k<256; k++) {
				int msb = 1<<(__builtin_ffs(k)-1);
				assert(k&msb);
				BB[kk][k][i] = BB[kk][k-msb][i] + BB[kk][msb][i];
			}
		}
	}
 ...
 *					subsetSumR2 - (size N/2) the sum of the exponents generating the R2 polynomials
 */
inline void calcSubsetSums (v8 subsetSumRqLogs[], R2poly* resR2, R2poly x)
{
#if SUBSET_TABLES
	...
	register R2poly R2;
#else
	register v8 R2_0, R2_1, R2_2, R2_3;
#endif
...
#if USE_NAIVE_R2
		// __builtin_prefetch(&CX[i][x8.u8[i]], 0, 0);
#else
		__builtin_prefetch(&BB[i][x8.u8[i]][0], 0, 0);
#endif
	}
#endif
	...
	for (int i=1; i<K/8; i++) {
#if USE_NAIVE_R2
		R2 = multiplyR2PolynomialsInRing(R2, CX[i][x8.u8[i]]);
#else
		R2_0 += BB[i][x8.u8[i]][0];
		R2_1 += BB[i][x8.u8[i]][1];
		R2_2 += BB[i][x8.u8[i]][2];
	...
	*resR2 = R2;
	...
	subsetSumR2Logs[0] = R2_0;
	subsetSumR2Logs[1] = R2_1;
	subsetSumR2Logs[2] = R2_2;
	subsetSumR2Logs[3] = R2_3;

	*resR2 = subsetSumOfExpsToR2Polynomial(subsetSumR2Logs);
#endif
	...
	v8* vecR2Ptr = (v8*)subsetSumR2Logs;
...
#define likely(x)    __builtin_expect (!!(x), 1)
	...
	while (likely(x[0])) {
		...
		vecR2Ptr[0] += vecBptr[0];
		vecR2Ptr[1] += vecBptr[1];
		vecRqPtr[4] += vecAptr[4];
		vecRqPtr[5] += vecAptr[5];
		vecRqPtr[6] += vecAptr[6];
		vecRqPtr[7] += vecAptr[7];
		vecR2Ptr[2] += vecBptr[2];
		vecR2Ptr[3] += vecBptr[3];
	}
 ...
 */
void updateXAndProducts (uint64_t* x, v16* resRq, R2poly* resR2, const uint64_t grayCodeCounter)
{
...
#if USE_CLMUL
	*resR2 = multiplyR2PolynomialsInRing (*resR2, C[changeIndex][inv]);
#else
	*resR2 = multiplyR2PolynomialsFromTable (*resR2, CC[changeIndex][inv]);
#endif
	// update x
	*x ^= mask;
...
uint64_t runOutputFeedbackMode (R2poly x, const int nIterations)
{
	...
	v8 subsetSumOfRqLogs [N/16] ALIGN;

	// the R_2 polynomial result coefficients
	R2poly polynomialR2;

	PackedBits res[8] ALIGN;

	// the calculated coefficients of the R_q result polynomial
	v16 outputQ[N/8] ALIGN;

	// each iteration computes 8 'phases' of the PRF function.
	// every output is 127 bits, which are then manipulated into 127 bytes of random output
	int64_t iter;
	for (iter = 0; iter <= nIterations/8; ++iter)
	{
		int phase;
		for (phase = 0; phase < 8; ++phase)
		{
			// get the initial subset-sums according to the x boolean array
			calcSubsetSums (subsetSumOfRqLogs, &polynomialR2, x);
			// the R_q polynomial multiplication output
			multiplyRqPolynomials (outputQ, subsetSumOfRqLogs);

			x = polynomialR2 ^ getQlsb(outputQ);
			res[phase] = x;

#if (PRINT_OUTPUT)
			printPolynomial(polynomialR2); printf (" ");
			printPolynomial((v64)outputQ[0]); printf (" [%04x] ", v8_extract_msb((v8)outputQ[0]));
			printPolynomial(getQlsb(outputQ)); printf (" ");
			printPolynomial(res[phase]); printf ("\n");
#endif

		}

		uint8_t* output = (uint8_t*)res;
		// output is a byte pointer
		// now points to 8 consecutive results of the function in OFB mode
		// pack 8 outputs into N-1 bytes (use only first N-1 bytes of output array)
		// in order to save in instructions, bits that should be moved are simply xored
		// to bits which should have been removed
		output[0] ^= (output[N-1] & 1);
		output[N/8] ^= ((output[N-1]>>1) & 1);
		output[2*N/8] ^= ((output[N-1]>>2) & 1);
		output[3*N/8] ^= ((output[N-1]>>3) & 1);
		output[4*N/8] ^= ((output[N-1]>>4) & 1);
		output[5*N/8] ^= ((output[N-1]>>5) & 1);
		output[6*N/8] ^= ((output[N-1]>>6) & 1);
	}

	uint64_t* funcOutputPtr = (uint64_t*)res;
	uint64_t finalOutput = 0;
	int i;
	for (i=0; i < N/8; ++i)
	{
		finalOutput ^= funcOutputPtr[i];
	}
	return finalOutput;
}
...
uint64_t runGrayCodeMode (uint64_t iv, const int nIterations)
{
	...
	// 8 consecutive outputs of the PRF in counter mode
	union u64 res[8];
	...
	// get the initial subset-sums according to the x boolean array
	union u64 IV = {.v={0, iv}};
	calcSubsetSums (subsetSumOfRqLogs, &polynomialR2, IV.v);
	...
	// each iteration computes 8 'phases' of the PRF function.
	// every output is N-1 bits, which are then manipulated into N-1 bytes of random output
	int64_t iter;
	for (iter = 0; iter <= nIterations/8; ++iter)
	{
		...
		for (phase = 0; phase < 8; ++phase)
		{
			...
			res[phase].v = polynomialR2 ^ getQlsb(outputQ);

#if (PRINT_OUTPUT)
			printPolynomial(res[phase].v);	printf ("\n");
#endif

			// update the gray-code counter and change x and the subset-sums accordingly
			++grayCodeCounter;
			updateXAndProducts (&x, resRq, &polynomialR2, grayCodeCounter);
		}

		uint8_t* output = (uint8_t*)res;
		// output is a byte pointer
		// now points to 8 consecutive results of the function in gray-code CTR mode
		// pack 8 outputs into N-1 bytes (use only first N-1 bytes of output array)
		// in order to save in instructions, bits that should be moved are simply xored
		// to bits which should have been removed
		output[0] ^= (output[N-1] & 1);
		...
		// this is just to force the compiler not to skip any computation of the output
		int i;
		for (i=0; i < 8; ++i)
		{
			finalOutput ^= res[i].u[0] ^ res[i].u[1];
		}
...
int main ()
{
	...
	// intialize precomputed AA and BB
	initializeXX();

	printf ("SPRING-CRT: N = %d	 K = %d\n\n", N, K);
