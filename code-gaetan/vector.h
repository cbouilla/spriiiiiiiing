...
/***********************************************************************
 * This files defines a set of macros which implements a set of vector
 * instructions (initially used to implement the SIMD hash function).
 *
...
#ifdef __SSE4_1__
#define v8_signextend_16 __builtin_ia32_pmovsxbw128
#endif
#define v16_saturate_8 __builtin_ia32_packsswb128
#define v8_extract_msb __builtin_ia32_pmovmskb128

#define v8_extract_bits(v,t0,t1,t2,t3,t4,t5,t6,t7,			\
			  t8,t9,t10,t11,t12,t13,t14,t15) ({		\
      static const union cv8 mask = {.u8 = {				\
	  1<<t0,1<<t1,1<<t2,1<<t3,1<<t4,1<<t5,1<<t6,1<<t7,1<<t8,	\
	  1<<t9,1<<t10,1<<t11,1<<t12,1<<t13,1<<t14,1<<t15}};		\
      v8 x = v&mask.v8;							\
      v8_extract_msb(-x);						\
    })

#define v8_extract_bit(v,n) ({						\
      v8 x = (v8)v32_shift_l((v32)v,7-n);				\
      v8_extract_msb(x);						\
    })
...
#define v16u_shift_r  __builtin_ia32_psrlwi128
...
#define vec_eq(a,b)      (v8_extract_msb(__builtin_ia32_pcmpeqb128((v8)(a), (v8)(b))) == 0xffff)
#define v64_eq vec_eq
...
#define v64_clmul __builtin_ia32_pclmulqdq128
// #define v64_rotate_bytes(x,n) _mm_alignr_epi8((x),(x),n)
#define v64_rotate_bytes(x,n) __builtin_ia32_palignr128((x),(x),8*(n))
#define merge_align(a,b) __builtin_ia32_palignr128(a,b,64)
...
#define v64_clmul _mm_clmulepi64_si128
#define v64_rotate_bytes(x,n) _mm_alignr_epi8((x),(x),n)
...
#define PERM_WITH_LOADS
#define V64_USE_HALF


#if !defined(USE_INLINE_ASSEMBLY)
...
#define v64_xor_half(x, h, l)                              \
  v64_merge(                                               \
            veor_s64(vget_low_s64(x) , vget_high_s64(l)),  \
            veor_s64(vget_high_s64(x), vget_low_s64(h) )   \
           )
...
#define v16u_shift_r  vshrq_n_u16

#define v32_shift_r  vshrq_n_u32
#define v32_shift_l  vshlq_n_u32

#define v64_shift_r(x,n)  vreinterpretq_s64_u64(vshrq_n_u64(x,n))
#define v64_shift_l  vshlq_n_u64

#define v16_cmp(a,b) vreinterpretq_s16_u16(vcgtq_s16(a,b))

#define v8_extract_bits(v,t0,t1,t2,t3,t4,t5,t6,t7,t8,t9,t10,t11,t12,t13,t14,t15) \
  ({									\
    const uint8x16_t test__ = {						\
      1<<t0,1<<t1,1<<t2,1<<t3,1<<t4,1<<t5,1<<t6,1<<t7,1<<t8,		\
      1<<t9,1<<t10,1<<t11,1<<t12,1<<t13,1<<t14,1<<t15};			\
    const uint8x8_t mask__ = {0x01,0x02,0x04,0x08,			\
			      0x10,0x20,0x40,0x80};			\
    v16 a__ = vtstq_u8(v, test__);					\
    int8x8_t b1__ = vand_u8(vget_low_u8(a__),mask__);			\
    int8x8_t b2__ = vand_u8(vget_high_u8(a__),mask__);			\
    int16x4_t c1__ = vpaddl_u8(b1__);					\
    int16x4_t c2__ = vshl_n_s16(vpaddl_u8(b2__),8);			\
    vget_lane_u16(vreinterpret_u16_s64(vpaddl_s32(vpaddl_u16(veor_s16(c1__,c2__)))),0); \
  })

#define v8_extract_bit(v,t) v8_extract_bits(v,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t,t)
#define v8_extract_msb(v) v8_extract_bit(v,7)

#define v16_saturate_8(a,b) vcombine_s8(vqmovn_s16(a),vqmovn_s16(b))

#define v64_eq(a,b)      (v8_extract_msb(vceqq_u8(vreinterpretq_s64_u8(a), vreinterpretq_s64_u8(b))) == 0xffff)
...
/* #define  v64_shufrot_inplace_2(a__,b__) ({                      \ */
/*       poly8x16x2_t c__ = vzipq_p8 (vreinterpretq_p8_s32(a__), vreinterpretq_p8_s32(b__));	\ */
/*       a__ = vreinterpretq_s32_p8(c__.val[0]);					\ */
/*       b__ = vreinterpretq_s32_p8(c__.val[1]);					\ */
/*     }) */
...
#define v64_rotate_bytes(x,n)	\
  vreinterpretq_u8_s64(vextq_u8(vreinterpretq_s64_u8(x),vreinterpretq_s64_u8(x),n))
...
#define v32_rotate(x,n)				\
  v32_or(					\
	 vshlq_n_s32(x, n),			\
	 vreinterpretq_s32_u32(vshrq_n_u32(vreinterpretq_u32_s32(x), 32-(n))))


#define v32_rotate_16(x) vreinterpretq_s32_s16(vrev32q_s16(vreinterpretq_s16_s32(x)))
...
#define v64_rotate(x,n)				\
  v64_or(					\
	 vshlq_n_s64(x, n),			\
	 vreinterpretq_s64_u64(vshrq_n_u64(vreinterpretq_u64_s64(x), 64-(n))))


#define v64_xrotate(x,y,n) ({                                   \
      v64 t__, u__, v__, w__;                                           \
      t__ = vshlq_n_s64(x, n);                                          \
      v__ = vshlq_n_s64(y, n);                                          \
      u__ = vreinterpretq_s64_u64(vshrq_n_u64(vreinterpretq_u64_s64(x), 64-(n))); \
      w__ = vreinterpretq_s64_u64(vshrq_n_u64(vreinterpretq_u64_s64(y), 64-(n))); \
      y = v64_merge(vorr_s64(vget_high_s64(t__),vget_high_s64(u__)), vorr_s64(vget_low_s64(v__),vget_low_s64(w__))); \
      x = v64_merge(vorr_s64(vget_high_s64(v__),vget_high_s64(w__)), vorr_s64(vget_low_s64(t__),vget_low_s64(u__))); \
    })

#define v64_yrotate(x,y,n) ({                                   \
      v64 t__, u__, v__, w__;                                           \
      t__ = vshlq_n_s64(x, n);                                          \
      v__ = vshlq_n_s64(y, n);                                          \
      u__ = vreinterpretq_s64_u64(vshrq_n_u64(vreinterpretq_u64_s64(x), 64-(n))); \
      w__ = vreinterpretq_s64_u64(vshrq_n_u64(vreinterpretq_u64_s64(y), 64-(n))); \
      x = v64_merge(vorr_s64(vget_high_s64(t__),vget_high_s64(u__)), vorr_s64(vget_low_s64(v__),vget_low_s64(w__))); \
      y = v64_merge(vorr_s64(vget_high_s64(v__),vget_high_s64(w__)), vorr_s64(vget_low_s64(t__),vget_low_s64(u__))); \
    })
...
#define v32_lswap(x) vreinterpretq_s32_s8(vrev32q_s8(vreinterpretq_s8_s32(x)))

#define v64_lswap(x) V864(vrev64q_s8(V648(x)))
//vextq_s64(x,x,1)

#define vv64_lswap(x) vreinterpret_s64_s8(vrev64_s8(vreinterpret_s8_s64(x)))

#define v64_bswap(x) (x)
